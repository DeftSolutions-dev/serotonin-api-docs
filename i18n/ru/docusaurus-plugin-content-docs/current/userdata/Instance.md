...
:::note Формат возврата перепроверен на `version-460909c4fe904aae`
Changelog клиента Serotonin перечисляет корректировку формата для `LookVector` / `RightVector` / `UpVector`. Перепроверено вживую на этом билде: все три возвращают стандартный `Vector3` userdata, идентичный любому другому vector-свойству. `tostring` даёт `"%.6f, %.6f, %.6f"` (камера, смотрящая вперёд, печатает `"-0.000000, -0.000000, -1.000000"`); `.X` / `.Y` / `.Z`, lowercase-алиасы `.x` / `.y` / `.z`, `.Magnitude` и `.Unit` все резолвятся; числовая индексация (`v[1]`) возвращает `nil`. `cam.CFrame` по-прежнему `nil`.
:::
