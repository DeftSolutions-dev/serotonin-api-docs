...
:::note Среда
Этот reference документирует отдельную sandboxed LuaJIT 2.0.3 среду со своими биндингами. Это не официальный Roblox/Luau API и не Roblox Studio: сниппеты отсюда работают только внутри этого сэндбокса и не запустятся в Roblox Studio. `jit` / `ffi` / `os` / `io` вырезаны. Каждая сигнатура проверена вживую против рантайма, а не взята на веру из Roblox-доков.
:::
